...
def upload_data():
    try:
        ...
        # Perform operations with the loaded data if needed
        # For example, update the existing 'users' data
        # users.update(loaded_data)

        # Convert the updated user data to DataFrame
        updated_valores = pd.DataFrame(users)
        updated_valores = updated_valores.fillna(0)
        updated_users_dict = updated_valores.to_dict(orient='index')

        return jsonify({"message": "Binary data uploaded successfully"})
...
'''peliculas = pd.read_csv("peli.bin" , sep=",")
